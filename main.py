...
if is_kaggle:
    ...
    output = "/home/rafael/repo/AnomalyDetection/Data/output"

print("dicom to file")
DataLoader.dicom_to_file(path=path_train,
                         output_path=output,
                         size=size)
DataLoader.dicom_to_file(path=path_test,
                         output_path=output,
                         size=size)

datagen = tf.keras.preprocessing.image.ImageDataGenerator(rescale=1. / 255., validation_split=0.20)

print("train")
train_ap = datagen.flow_from_directory(os.path.join(output),
                                       class_mode="input",
                                       batch_size=batch,
                                       classes=["AP"])
train_pa = datagen.flow_from_directory(os.path.join(output),
                                       class_mode="input",
                                       batch_size=batch,
                                       classes=["PA"])
